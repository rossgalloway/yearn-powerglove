...
export function VaultsProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: strategiesData, loading: strategiesLoading, error: apolloError2 } = useQuery(GET_STRATEGIES)
  const { assets, loading: assetsLoading, error: assetsError } = useTokenAssets() // fetch token assets
  const { data: yDaemonVaults, isLoading: yDaemonLoading, error: yDaemonError } = useYDaemonVaults()
  ...
  const loadingState = useMemo(() => {
    ...
    const isInitialLoading = isVaultsLoading && isStrategiesLoading && isAssetsLoading && isYDaemonLoading

    // Any query is still loading
    const isPartialLoading = isVaultsLoading || isStrategiesLoading || isAssetsLoading || isYDaemonLoading
  ...
  const globalError = apolloError || apolloError2 || assetsError || yDaemonError || null
  ...
  const yDaemonMap = useMemo(() => {
    ...
    const map = new Map<string, { forwardApy?: number | null; strategyAprs: Record<string, number | null> }>()
  ...
  const normalizedVaults = useMemo(() => applyVaultOverrides(enrichedVaults), [enrichedVaults])
