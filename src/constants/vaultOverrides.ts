...
export type VaultOverrideValues = Partial<Omit<VaultExtended, 'meta' | 'tvl' | 'apy'>> & {
...
export const buildVaultOverrideKey = (chainId: ChainId, address: string) => `${chainId}-${address.toLowerCase()}`
...
export const VAULT_OVERRIDE_ENTRIES: VaultOverrideConfig[] = [
  {
    ...
    chainId: 1,
    blacklist: false,
    blacklistReason: 'One of the underlying constituents of the pool (yETH) was exploited.',
    overrideReason:
...
export const VAULT_OVERRIDES: Record<string, VaultOverrideConfig> = VAULT_OVERRIDE_ENTRIES.reduce(
  (acc, entry) => {
