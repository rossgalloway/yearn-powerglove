...
export function useVaultListData(vaults: Vault[], tokenAssets: TokenAsset[]): VaultListData[] {
  ...
  return useMemo((): VaultListData[] => {
    return vaults.map((vault) => {
      const apyRawValue = vault.apy?.monthlyNet ?? vault.apy?.net ?? 0
      return {
        ...
        APY: formatApyDisplay(apyRawValue), // Prefer 30-day APY when available
        apySortValue: normalizeApyDisplayValue(apyRawValue),
        apyRawValue: apyRawValue * 100,
