...
import { type VaultListData, VaultRow } from '@/components/vaults-list/VaultRow'
...
export const VaultsTable: React.FC<VaultsTableProps> = React.memo(({ vaults, availableHeight }) => {
  ...
  if (vaults.length === 0) {
    return (
      ...
      >
        <div>
          <p className="text-lg font-medium mb-2">No vaults found with those filters.</p>
  ...
  return (
    <VirtualScrollTable
      ...
      renderItem={(vault: VaultListData) => <VaultRow key={`${vault.chain}-${vault.id}`} vault={vault} />}
      className="border-0"
